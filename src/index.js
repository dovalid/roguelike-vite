...
window.addEventListener("keydown", (event) => {
  ...
  if (!interrupted && !path) {
    ...
    if (direction) game.input("move", direction);
    else if (key === "c") game.input("close");
    else if (code === "Space") game.input("wait");
    else if (key === ">") game.input("descend");
    else if (key === "<") game.input("ascend");
    else if (key === "r") game.start();
  }
});
...
canvas.addEventListener("click", (event) => {
  ...
  if (Cell.isEqual(cell, mouse)) {
    ...
    if (id === ENTRANCE) game.input("ascend");
    else if (id === EXIT) game.input("descend");
    return;
  }
  let cells = {};
